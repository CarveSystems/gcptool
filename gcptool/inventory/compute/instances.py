...
def all(project_id: str, cache: Cache) -> List[Instance]:
    return [Instance(**instance) for by_zone in __all(cache, project_id).values() for instance in by_zone]
...
def by_zone(project_id: str, cache: Cache) -> Dict[str, List[Instance]]:
    ...
    for zone,zone_instances in __all(cache, project_id).items():
        instances[zone] = [Instance(**instance) for instance in zone_instances]
    return instances
